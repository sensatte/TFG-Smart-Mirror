...
class MenuLayout(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        widgetList = ['schedule', "news", "settings", "at",
                      "clock", "schedule", "settings", "settings", "settings"]
        ...
        for i in range(self.maxCols):
            for j in range(self.maxRows):
                self.itemsMatrix[(i, j)] = MenuOption(name=widgetList[ind], imageUri="images\\menu\\" +
                                                      widgetList[ind] + ".png", maxCols=self.maxCols, maxRows=self.maxRows, posTuple=(i, j))
                ind += 1
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        ...
        if keycode[1] == 'enter':
            self.pressedOption(
                self.itemsMatrix[(self.selectedItem[0], self.selectedItem[1])])
...
class MenuOption(Image):
    ...
    def __init__(self, name, posTuple, maxCols, maxRows, imageUri):
        ...
        self.pos_hint = {'center_x': c *
                         (posTuple[0])+c/2, 'center_y': r * (posTuple[1])+r/2}
