...
class GymConfig(Screen):
    ...
    def saveWeight(self):
        ...
        peso = float(dic[self.ids.box] + dic[self.ids.diez] +
                     dic[self.ids.uni])
    ...
    def start(self, widget):
        anim = Animation(pos_hint={"center_x": .25, "y": .97}, duration=.1)
        anim += Animation(pos_hint={"center_x": .25, "y": 1}, duration=.1)
        anim.start(widget)
    ...
    def stop(self, widget):
        anim = Animation(pos_hint={"center_x": .5, "y": .97}, duration=.1)
        anim += Animation(pos_hint={"center_x": .5, "y": 1}, duration=.1)
        anim.start(widget)
        Clock.unschedule(self.increment_time)

    # To start the count
    def restart(self, widget):
        anim = Animation(pos_hint={"center_x": .75, "y": .97}, duration=.1)
        anim += Animation(pos_hint={"center_x": .75, "y": 1}, duration=.1)
        anim.start(widget)
        self.number = 0
        Clock.unschedule(self.increment_time)
